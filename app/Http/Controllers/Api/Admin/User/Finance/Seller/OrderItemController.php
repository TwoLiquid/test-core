...
use App\Exports\Api\Admin\User\Finance\Seller\OrderItemExport;
use App\Http\Controllers\Api\Admin\User\Finance\Seller\Interfaces\OrderItemControllerInterface;
...
use App\Transformers\Api\Admin\User\Finance\Seller\OrderItem\OrderItemListPageTransformer;
    ...
    ) : JsonResponse
    {
        ...
        if (!$user) {
            return $this->respondWithError(
                trans('validations/api/admin/user/finance/seller/orderItem/index.result.error.find')
            );
        }
         ...
         */
        $orderItems = $this->orderItemRepository->getAllForSellerFiltered(
            $user,
         ...
         */
        $orderItemsForLabels = $this->orderItemRepository->getAllForSellerFilteredForAdminLabels(
            $user,
            $request->input('order_item_id'),
            $request->input('date_from'),
            $request->input('date_to'),
         ...
         */
        $invoiceStatuses = $this->invoiceService->getByOrderItemsForAdminSellerStatusesByIds(
            $orderItemsForLabels
        );
        ...
        if ($request->input('paginated') === true) {
            ...
            return $this->setPagination($paginatedOrderItems)->respondWithSuccess(
                ...
                trans('validations/api/admin/user/finance/seller/orderItem/index.result.success')
            );
        }
