...
class VybeChangeRequestAppearanceCaseTransformer extends BaseTransformer
{
    ...
    public function transform(VybeChangeRequestAppearanceCase $vybeChangeRequestAppearanceCase) : array
    {
        ...
        $language = $vybeChangeRequestAppearanceCase->vybeChangeRequest
            ->vybe
            ->user
            ->getLanguage();
        ...
        return [
            ...
            'unit_suggestion'        => $vybeChangeRequestAppearanceCase->unit_suggestion ? [
                $language->iso => $vybeChangeRequestAppearanceCase->unit_suggestion
            ] : null,
            ...
            'previous_city_id'       => $vybeChangeRequestAppearanceCase->previous_city_place_id,
    ...
    public function includeUnit(VybeChangeRequestAppearanceCase $vybeChangeRequestAppearanceCase) : ?Item
    {
        ...
        if ($vybeChangeRequestAppearanceCase->relationLoaded('unit')) {
     ...
     * @return Item|null
     */
    public function includePreviousUnit(VybeChangeRequestAppearanceCase $vybeChangeRequestAppearanceCase) : ?Item
    {
        $previousUnit = null;

        if ($vybeChangeRequestAppearanceCase->relationLoaded('previousUnit')) {
            $previousUnit = $vybeChangeRequestAppearanceCase->previousUnit;
        }

        return $previousUnit ? $this->item($previousUnit, new UnitTransformer) : null;
    }
    ...
    public function includeUnitCsauStatus(VybeChangeRequestAppearanceCase $vybeChangeRequestAppearanceCase) : ?Item
    {
        ...
        if ($vybeChangeRequestAppearanceCase->relationLoaded('csauSuggestion')) {
            $csauSuggestion = $vybeChangeRequestAppearanceCase->csauSuggestion;
     ...
     * @throws DatabaseException
     */
    public function includePreviousPlatforms(VybeChangeRequestAppearanceCase $vybeChangeRequestAppearanceCase) : ?Collection
    {
        $previousPlatforms = null;

        if ($vybeChangeRequestAppearanceCase->previous_platforms_ids) {
            $previousPlatforms = $this->platformRepository->getByIds(
                $vybeChangeRequestAppearanceCase->previous_platforms_ids
            );
        }

        return $previousPlatforms ? $this->collection($previousPlatforms, new PlatformTransformer) : null;
    }
