...
use App\Repositories\Vybe\Interfaces\VybeDeletionRequestRepositoryInterface;
use Carbon\Carbon;
...
/**
 * Class VybeDeletionRequestRepository
 *
 * @package App\Repositories\Vybe
 */
class VybeDeletionRequestRepository extends BaseRepository implements VybeDeletionRequestRepositoryInterface
{
    /**
     * VybeDeletionRequestRepository constructor
     */
    public function __construct()
    {
        $this->cacheTime = config('repositories.vybeDeletionRequest.cacheTime');
        $this->perPage = config('repositories.vybeDeletionRequest.perPage');
    }
    ...
    {
        try {
            return VybeDeletionRequest::query()
                ->find($id);
        } catch (Exception $exception) {
            throw new DatabaseException(
                trans('exceptions/repository/vybe/vybeDeletionRequest.' . __FUNCTION__),
    ...
    {
        try {
            return Cache::remember('vybeDeletionRequests.all.count', $this->cacheTime,
                function () {
                    return VybeDeletionRequest::query()
                        ->count();
                }
    ...
    {
        try {
            return VybeDeletionRequest::query()->create([
                'vybe_id'            => $vybe->id,
                'message'            => $message,
                'status_id'          => VybeStatusList::getDeletedItem()->id,
    ...
    {
        try {
            return $vybeDeletionRequest->delete();
        } catch (Exception $exception) {
            throw new DatabaseException(
                trans('exceptions/repository/vybe/vybeDeletionRequest.' . __FUNCTION__),
                $exception->getMessage()
